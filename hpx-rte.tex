...
\item \textbf{C and C++ Compatibility}\\
  The majority of Open MPI source code is written in C language. HPX is completely written in C++ with extensive use of recent C++14 features. This definitely causes compatibility issues which needed to be handled in the code. For instance, Listing \ref{lst:headers} illustrates an example on how different headers are made visible to either C or C++ compiler. 
