...
\item \textbf{Intercepting printf}\\
  In order to see the output from C part of the code, we needed to intercept the printf function in C and send its output to C++ output stream.
\item \textbf{Automatic Boost and HPX Detection}\\
  Since Boost and HPX are required to be installed before the installation of Open MPI with HPX-RTE, we modified the configure logic of Open MPI to detect the installation paths of these libraries and accordingly modify the generated make files.
\item \textbf{C and C++ Compatibility}\\
  The majority of Open MPI source code is written in C language. HPX is completely written in C++ with extensive use of recent C++14 features. This definitely causes compatibility issues which needed to be handled in the code. For instance, the header file is Listing \ref{lst:rte_hpx_db} illustrates how different headers and function prototypes are made visible to either C or C++ compiler. As another example, we had to use C++ to compile a number of components in Open MPI.
  \begin{lstlisting}[language=C, frame=single, basicstyle=\footnotesize, caption=rte_hpx_db.h\label{lst:rte_hpx_db}]
