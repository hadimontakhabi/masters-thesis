...
Design and implementation of HPX-RTE, the new runtime environment for Open MPI, is the main contribution of this thesis. HPX-RTE provides a compatibility layer for MPI applications to run on HPX runtime environment without any modifications to the applications written in MPI. HPX-RTE facilitates the transition from MPI to ParalleX programming model. HPX-RTE makes it possible for new programming paradigms such as MPI-HPX hybrid model to emerge. This thesis also presents a set of priliminary performance comparisons between HPX-RTE and ORTE (Open MPI's default runtime environment).
